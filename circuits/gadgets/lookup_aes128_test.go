...
func TestLookUpAES128(t *testing.T) {
	assert := test.NewAssert(t)

	fmt.Println("testing aes in gnark using")
	...
	keyAssign := LookUpStrToIntSlice(key, true)
	ptAssign := LookUpStrToIntSlice(plaintext, true)
	ctAssign := ciphertext // StrToIntSlice(ciphertext, true)
	nonceAssign := LookUpStrToIntSlice(Nonce, true)
...
func LookUpStrToIntSlice(inputData string, hexRepresentation bool) []int {
	var byteSlice []byte
